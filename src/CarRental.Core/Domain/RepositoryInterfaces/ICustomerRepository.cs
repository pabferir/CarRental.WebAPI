...
namespace CarRental.Core.Domain.RepositoryInterfaces
{
    ...
    {
        Task<Customer> InsertCustomer(string identityNumber, string name, string surname, DateTime dateOfBirth, string telephoneNumber, bool saveChanges);
        Task<IEnumerable<Customer>> GetCustomerWhere(Expression<Func<Customer, bool>>? filter);
        Task<Customer> UpdateCustomer(Guid id, string identityNumber, string name, string surname, DateTime dateOfBirth, string telephoneNumber, bool saveChanges);
